...
#SBATCH --job-name=vit_6-6_cifar100                         # sets the job name
#SBATCH --output=vit_6-6_cifar100.out.%j                    # indicates a file to redirect STDOUT to; %j is the jobid. If set, must be set to a file instead of a directory or else submission will fail.
#SBATCH --error=vit_6-6_cifar100.out.%j                     # indicates a file to redirect STDERR to; %j is the jobid. If set, must be set to a file instead of a directory or else submission will fail.
...
OUTPUT_DIR='/fs/nexus-scratch/vla/ViT_model_weights/6-6/cifar100'
...
python train_n_bit.py --name 6-6_cifar100 \
                      --model_type=ViT-B_16 \
                      --dataset cifar100 \
