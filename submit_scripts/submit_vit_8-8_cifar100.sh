...
#SBATCH --job-name=vit_8-8_cifar100                         # sets the job name
#SBATCH --output=vit_8-8_cifar100.out.%j                    # indicates a file to redirect STDOUT to; %j is the jobid. If set, must be set to a file instead of a directory or else submission will fail.
#SBATCH --error=vit_8-8_cifar100.out.%j                     # indicates a file to redirect STDERR to; %j is the jobid. If set, must be set to a file instead of a directory or else submission will fail.
...
OUTPUT_DIR='/fs/nexus-scratch/vla/ViT_model_weights/8-8/cifar100'
...
python train_n_bit.py --name 8-8_cifar100 \
                      --model_type=ViT-B_16 \
                      --dataset cifar100 \
