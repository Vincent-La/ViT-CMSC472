...
#SBATCH --job-name=vit_4-4_cifar10                         # sets the job name
#SBATCH --output=vit_4-4_cifar10.out.%j                    # indicates a file to redirect STDOUT to; %j is the jobid. If set, must be set to a file instead of a directory or else submission will fail.
#SBATCH --error=vit_4-4_cifar10.out.%j                     # indicates a file to redirect STDERR to; %j is the jobid. If set, must be set to a file instead of a directory or else submission will fail.
...
OUTPUT_DIR='/fs/nexus-scratch/vla/ViT_model_weights/4-4/cifar10'
...
python train_n_bit.py --name 4-4_cifar10 \
                      --model_type=ViT-B_16 \
                      --dataset cifar10 \
